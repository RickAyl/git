...
public class MetroJsonAction extends ServiceJson {
    ...
    private final String BLL_VIP = "bll_vip";

    private volatile String countryLanguage;
    
    /**when net connected or local changed,we must make sure delete DB 
     * data according to the right language**/
     ...
     * @throws JSONException
     */
    synchronized public void parsePgmJson(String language) {
        ...
        public void run() {
            synchronized (mLock) {
                ...
                while (bThreadFlag) {
                     ...
                     */
                    addRequestHeads();
                    String param = "ifid=GetLaucherTestUser&mac=" + getMacAddress();
                    try {
                        JSONObject vipObj = getJSONObject(param);
                        if (vipObj != null) {
                            if (vipObj.getInt("err") == 0) {
                                JSONObject bdJsonObject = vipObj.getJSONObject("bd");
                                SystemProperties.set(BLL_VIP, bdJsonObject.getString("cd"));
                                if (DEBUG) {
                                    Log.d(TAG, "bll vip request success");
                                }
                            } else {
                                if (DEBUG) {
                                    Log.d(TAG, "bll vip request failed");
                                }
                            }
                        }
