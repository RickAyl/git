...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public static final String APP_TABLE_NAME = "apppage";
    ...
    public static final String METROINFO_COLUMN_NET = "net";
    
    // black list tag
    public static final String METROINFO_COLUMN_HIDE = "hide";

    // 0没有限制,1不可移除,2不可弹出菜单
    public static final String METROINFO_COLUMN_MOVE_TYPE = "moveType";

    // 位置ID
    public static final String METROINFO_COLUMN_POSITION_ID = "positionID";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS responsehead (_id INTEGER PRIMARY KEY AUTOINCREMENT," + "error INT,"
                ...
                + "etag TEXT)");
        db.execSQL("CREATE TABLE IF NOT EXISTS apppage (AppID INTEGER PRIMARY KEY AUTOINCREMENT," + "TypeTitle TEXT,"
                + "AppTitle TEXT," + "ClassName TEXT," + "PkgName TEXT," + "Position_x INT," + "Position_y INT,"
                + "Width INT," + "Height INT," + "AppType INT," + "AppCategory INT," + "StrFlag TEXT," + "IntFlag INT,"
                + "IconPathB TEXT," + "IconPathF TEXT," + "AppURL TEXT,country TEXT,net INT, hide INT, moveType INT, positionID INT)");
    }
...
//        db.execSQL("drop table if exists responsehead");
        if (oldVersion == 4 && newVersion == 5) {
            db.execSQL("CREATE TABLE IF NOT EXISTS apppage (AppID INTEGER PRIMARY KEY AUTOINCREMENT," + "TypeTitle TEXT,"
                    ...
                    + "Width INT," + "Height INT," + "AppType INT," + "AppCategory INT," + "StrFlag TEXT," + "IntFlag INT,"
                    + "IconPathB TEXT," + "IconPathF TEXT," + "AppURL TEXT,country TEXT,net INT, hide INT, moveType INT, positionID INT)");
            db.execSQL("UPDATE metroinfos SET PkgName='com.eostek.launcher', ClassName='com.eostek.launcher.ui.AllAppActivity' where PkgName='com.eostek.dangle.m805';");
            db.execSQL("UPDATE metroinfos SET PkgName='com.eostek.scifly.widget' where PkgName='com.eostek.wasuwidgethost';");
        }
    }
    ...
    public long emptyDBData(String country) {
        ...
        long t4 = emptyTableData(APP_TABLE_NAME, country);
        return (t1 + t2 + t3 + t4);
    }
     ...
     * @return The cursor
     */
    public Cursor getAppPageCursor() {
        Cursor cursor = mDB.rawQuery("select * from apppage", null);
        return cursor;
    }
    ...
    /**
     * get the given country metro info
     * 
     * @since V1.2.1
     * @param country The country flag
     * @return The cursor
     */
    public Cursor getAppAddMetroPageCursor(String country) {
        Cursor cursor = mDB.rawQuery("select * from metroinfos where " + METROINFO_COLUMN_COUNTRY + "='" + country
                + "'", null);
        return cursor;
    }
    ...
    public int getAppTableCount(String country) {
        int i = 0;
        Cursor c = null;
        c = mDB.rawQuery("select count(*)as count from apppage where " + METROINFO_COLUMN_COUNTRY + "='" + country
                + "'", null);
        while (c.moveToNext()) {
            Log.d("sql", country + " c.getString(0):" + c.getString(0));
            i = Integer.parseInt(c.getString(0));
     ...
     * @return The cursor
     */
    public Cursor getMetroInfoCursor() {
        Cursor cursor = mDB.rawQuery("select * from metroinfos", null);
        return cursor;
    }
    
    public long insertAppMetroInfo(MetroInfo info) {
        long result = -1;
        ContentValues cv = new ContentValues();
        cv.put(METROINFO_COLUMN_TYPE_TITLE, info.getTypeTitle());
        cv.put(METROINFO_COLUMN_APPCATEGORY, info.getAppCategory());
        ...
        cv.put(METROINFO_COLUMN_ICON_APP_URL, info.getApkUrl());
        cv.put(METROINFO_COLUMN_COUNTRY, info.getCounLang());
        cv.put(METROINFO_COLUMN_NET, info.getNet());
        cv.put(METROINFO_COLUMN_HIDE, info.getHide());
        cv.put(METROINFO_COLUMN_MOVE_TYPE, info.getMoveType());
        cv.put(METROINFO_COLUMN_POSITION_ID, info.getPositionId());
        result = mDB.insert(APP_TABLE_NAME, null, cv);
        return result;
    }
    
    public int updateEnAddDBDate(MetroInfo info, int id) {
    	int result = -1;
        ContentValues cv = new ContentValues();
        cv.put(METROINFO_COLUMN_TYPE_TITLE, info.getTypeTitle());
        cv.put(METROINFO_COLUMN_APPCATEGORY, info.getAppCategory());
        ...
        cv.put(METROINFO_COLUMN_COUNTRY, info.getCounLang());
        cv.put(METROINFO_COLUMN_NET, info.getNet());
        Log.d("ahri","add app positionID =" + id);
        result = mDB.update(APP_TABLE_NAME, cv, "positionID =?",new String[]{"" + id});
    	
		return result;
    	
    }
    
	public int DeleteEnAppFromDB(String clsName) {
		int result = -1;
		result = mDB.delete(APP_TABLE_NAME, "ClassName =?", new String[]{"" + clsName});
		
		return result;
	}
	
	public void updateIdAndPosition(int id, int x, int y) {
		Log.d("ahri", "updateIdAndPosition id =" + id);
		mDB.execSQL("update " + APP_TABLE_NAME + " set positionID=positionID-1 where positionID>?", new String[]{"" + id});
		Cursor cursor = getAppPageCursor();
		cursor.moveToLast();
		ContentValues cv = new ContentValues();
		int lastId = cursor.getInt(cursor.getColumnIndex(DBHelper.METROINFO_COLUMN_POSITION_ID));
		Log.d("ahri", "lastId =" + lastId);
		for(int i = id;i <= lastId;i++){
			Log.d("ahri", "i =" + i);
			cv.put(METROINFO_COLUMN_POSITION_X,UIUtil.getEnXById(i));
			cv.put(METROINFO_COLUMN_POSITION_Y,UIUtil.getEnYById(i));
			mDB.update(APP_TABLE_NAME, cv, "positionID =?",new String[]{"" + i});
		}
		cursor.close();
	}
    
	public void updateEnAppID(int id){
		Log.d("ahri", "updateEnAppID id =" + id);
		Cursor cursor = getAppPageCursor();
		cursor.moveToLast();
		ContentValues cv = new ContentValues();
		int lastId = cursor.getInt(cursor.getColumnIndex(DBHelper.METROINFO_COLUMN_POSITION_ID));
		Log.d("ahri", "lastId  =" + lastId);
		for(int i = id;i <= lastId;i++){
			cv.put(METROINFO_COLUMN_POSITION_ID,(i - 1));
			cv.put(METROINFO_COLUMN_POSITION_X,UIUtil.getEnXById(i - 1));
			cv.put(METROINFO_COLUMN_POSITION_Y,UIUtil.getEnYById(i - 1));
			mDB.update(APP_TABLE_NAME, cv, "positionID =?",new String[]{"" + i});
		}
		cursor.close();
	}
