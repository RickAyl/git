...
import com.eostek.tv.launcher.ui.AllAppActivity;
...
public final class UIUtil {
    ...
    private static int mTitleListSize;
	...
	public static int getResourceId(Context mContext, String path) {
		int resID = mContext.getResources().getIdentifier(path, "drawable",
				"com.eostek.tv.launcher");
		return resID;
	}

	public static void startApkCibn(Context context, String pckName,
			String clsName, String extraStr, int extraInt) {
		Log.i(TAG, "startApkCibn");
		String temp = extraStr;
		String action = new String();
		String actionParam = new String();
		String[] param = temp.split("#=#");

		Intent i = new Intent();
		i.setComponent(new ComponentName("cn.cibntv.ott",
				"cn.cibntv.ott.Bootloader"));
		i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		
		if (4 == param.length) {
			action = param[2].substring(7, param[2].length());
			actionParam = param[3].substring(13, param[3].length());
			Log.i(TAG, action);
			Log.i(TAG, actionParam);
			Log.i(TAG,actionParam);
			i.putExtra("action", action);
			i.putExtra("actionParam",actionParam);
	 ...
	 * @param clsName
	 */
	public static void startApk(Context context, String pckName,
			String clsName, String extraStr, int extraInt) {
		...
		if (pckName.equals("cn.com.wasu.main") && !TextUtils.isEmpty(extraStr)) {
			...
			context.startActivity(intent);
		}else if(pckName.equals("cn.cibntv.ott")){
			Log.i(TAG, "startApkCibn");
			String temp = extraStr;
			String action = new String();
			String actionParam = new String();
			...
			if (4 == param.length) {
				action = param[2].substring(7, param[2].length());
				actionParam = param[3].substring(12, param[3].length());
				Log.i(TAG, action);
				Log.i(TAG, actionParam);
				Log.i(TAG,actionParam);
				i.putExtra("action", action);
			...
			intent.setComponent(componentName);
			if (clsName
					.equals("com.android.settings.Settings$DevelopmentSettingsActivity")) {
			...
			} else if (pckName.equals(LConstants.SCIFLY_91Q)) {
				if (!TextUtils.isEmpty(extraStr)) {
					// for 91Q
					Log.d(TAG, "extraStr =" + extraStr);
					intent.putExtra("code", extraStr);
	...
	public static void showToast(Context context) {
		Toast.makeText(context,
				context.getResources().getString(R.string.app_not_installed),
	 ...
	 * @param context
	 * @param filePath
	 *            The apk file path,like /sdcard/qq.apk
	 */
...
//		Toast.makeText(context, context.getResources().getString(R.string.apk_installing), Toast.LENGTH_SHORT).show();
//    	new Thread(new Runnable() {
//    		
//    		@Override
//    		public void run() {
//    			Log.d(TAG, "[installPackage]bInstalling=");
//    			
//    			PackageManagerExtra managerExtra = PackageManagerExtra.getInstance();
//    			Uri packageURI = Uri.fromFile(new File(filePath));
//    			managerExtra.installPackage(context, packageURI, new IPackageInstallListener() {
//    				
//    				
//    				@Override
//    				public void packageInstalled(String packageName, int returnCode) {
//    					Log.d(TAG, "Listener : >>>PackageManager PackageName : " + packageName + " ReturnCode : "
//    							+ returnCode);
//    					if (returnCode == 1) {
//    						// install success
//    					} else {
//    						Toast.makeText(context,context.getResources().getString(R.string.install_fail), Toast.LENGTH_SHORT).show();
//    					}
//    				}
//    			});
//    		}
//    	}).start();
	...
	public static int[] getScreenHieghtPX(Context context) {
		...
		return new int[] { wScreen, hScreen };
	}
	...
	public static boolean getNetworkState(String url) {
		...
		httpClient.getParams().setIntParameter(HttpConnectionParams.SO_TIMEOUT,
				LConstants.NETWORK_READ_DATA_TIMEOUT);
		try {
	...
	public static void setSystemUIVisible(Context context, int value) {
		...
		try {
			...
			Method setVisibleMethod = claz.getMethod("setSystemUiVisibility",
					paramTypes);
			// Parameters
	...
	public static boolean getBooleanFromXml(Context ctx, String key) {
		SharedPreferences sPreferences = ctx.getSharedPreferences(
				LAUNCHER_PREF, Context.MODE_PRIVATE);
	...
	public static void viewScaleUp(Context context, View view, float xValue,
			float yValue) {
		ScaleAnimation inAnimation = new ScaleAnimation(1.0f, xValue, 1.0f,
				yValue, ScaleAnimation.RELATIVE_TO_SELF, SCALE_CENTER,
				ScaleAnimation.RELATIVE_TO_SELF, SCALE_CENTER);
		inAnimation.setDuration(context.getResources().getInteger(
				R.integer.scale_animation_duration));
		inAnimation.setFillAfter(true);
		view.startAnimation(inAnimation);
	}

	public static void viewScaleDown(Context context, View view, float xValue,
			float yValue) {
		ScaleAnimation inAnimation = new ScaleAnimation(xValue, 1.0f, yValue,
				1.0f, ScaleAnimation.RELATIVE_TO_SELF, SCALE_CENTER,
				ScaleAnimation.RELATIVE_TO_SELF, SCALE_CENTER);
		inAnimation.setDuration(context.getResources().getInteger(
				R.integer.scale_animation_duration));
		inAnimation.setFillAfter(true);
	...
	public static void uploadNetworkDataError(Context context, String log) {
		if (isSystemApp(context, context.getPackageName())) {
			String title = "ETag:" + getStringFromXml(context, "ETag") + ";"
					+ log;
	...
	public static String getLanguage() {
		...
		if (lan.equals("fr") && country.equals("FR")) {
			lan = "en";
			country = "US";
		}
	...
	public static boolean shouldKillMemory(Context context) {
		...
		long percentUsed = (mi.totalMem - mi.availMem) * LConstants.HUNDRED
				/ mi.totalMem;
	...
	public static void setTitleList(int listSize) {
		mTitleListSize = listSize;
	}

	public static int getTitleList() {
		return mTitleListSize;
	}

	public static void startApk(Context ctx, String pkgName, String clsName) {
	...
	public static int getEnXById(int posId) {
		return 2944 + 178 * ((posId - 1) / LConstants.APP_ROW_NUM);
	}

	public static int getEnYById(int posId) {
		return ((posId - 1) % LConstants.APP_ROW_NUM) * 178;
	}
}
