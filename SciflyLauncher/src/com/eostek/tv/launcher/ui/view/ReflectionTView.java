...
public class ReflectionTView extends RelativeLayout {
    ...
    public ReflectionTView(Context context, ObservableScrollView scrollView) {
        super(context);
        /*try {
        	hasTuner = TvManager.getInstance().getFactoryManager().getTunerStatus();
        	Log.i(TAG, String.valueOf(hasTuner));
    ...
    public void notifiDateSetChange() {
        if (UIUtil.getSpecialCode().equals(LConstants.FEATURE_BENQ_828)) {
            sourceList = mContext.getResources().getStringArray(R.array.source_array_828_hastuner);
        } else if (UIUtil.getSpecialCode().equals(LConstants.FEATURE_638)) {
            sourceList = mContext.getResources().getStringArray(R.array.source_array_638);
        } else if(UIUtil.getSpecialCode().equals(LConstants.FEATURE_LX828)){
        	sourceList = mContext.getResources().getStringArray(R.array.source_array_lx828);
        }else {
        	if (!hasTuner) {
            	sourceList = mContext.getResources().getStringArray(R.array.source_array_828_notuner);
            }else{
            	sourceList = mContext.getResources().getStringArray(R.array.source_array_828_hastuner);
            }
        }
        ...
        if (UIUtil.getLanguage().equals("en-us")) {
            ...
            Log.d("ahri","mHandler.sendEmptyMessage");
        }
...
//        reflection.recycle();
    ...
    private class SouceItemSelectedListener implements OnItemSelectedListener {
        ...
        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
            ...
            if (LConstants.FEATURE_638.equals(UIUtil.getSpecialCode())) {
                if (sourceAdapter.signalStatus_638[position]) {
            	...
            	if (!hasTuner) {
            		 if (sourceAdapter.signalStatus_notuner[position]) {
                         tv.setTextColor(mContext.getResources().getColor(R.color.text_white));
                     } else {
                         tv.setTextColor(mContext.getResources().getColor(R.color.text_grey));
                     }
                }else{
                	if (sourceAdapter.signalStatus_hastuner[position]) {
                		tv.setTextColor(mContext.getResources().getColor(R.color.text_white));
                	} else {
                		tv.setTextColor(mContext.getResources().getColor(R.color.text_grey));
                	}
    ...
    private void setListViewParams() {
        mListViewParams = new RelativeLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.MATCH_PARENT);
        mListViewParams.height = 300;
        mListViewParams.topMargin = 320;
