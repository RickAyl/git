...
//                    setWidgetReflect(hostView);
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                ...
                if (hasFocus) {
                	...
                	if(LConstants.FEATURE_BENQ_828.equals(UIUtil.getSpecialCode())){
                		if(ReflectImage.this.getMetroInfo().getY() == 310){
                			Log.d(TAG, "platform is BenQ 828(set db 1)");
                    ...
                    if (mParentGroup != null) {
                        ...
                        mParentGroup.setFocusedItem(metroInfo);
                    }
    ...
    /**
     * set the app bitmap ReflectImage
     * 
     * @param bmp The origin bitmap
     * @param title
     */
    public void setImgResource(Bitmap bmp, CharSequence title) {
        ((LinearLayout) mOriginFrameLayout.findViewById(R.id.app_layout)).setVisibility(View.VISIBLE);
        ((ImageView) mOriginFrameLayout.findViewById(R.id.icon)).setImageBitmap(bmp);
        ((TextView) mOriginFrameLayout.findViewById(R.id.title)).setText(title);
    }
...
//                    mHandler.sendEmptyMessageDelayed(GET_WIDGET_REFLECTION, 200);
    ...
    private class OriginButtonOnClickListener implements OnClickListener {
        ...
        public void onClick(View v) {
        	...
        	//once app is added to HomePage,click it to start apk
            if (metroInfo.getItemType() == 9) {
                Intent intent = new Intent();
                intent.setClassName(metroInfo.getPkgName(), metroInfo.getClsName());
                if (intent != null) {
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    try {
                        mContext.startActivity(intent);
                    } catch (ActivityNotFoundException anf) {
                        Toast.makeText(mContext, "apprunerror", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(mContext, "apprunerror", Toast.LENGTH_SHORT).show();
                }
            ...
            if (pkgName == null || clsName == null || TextUtils.isEmpty(pkgName) || TextUtils.isEmpty(clsName)) {
                Log.d(TAG, "the pkgName or clsName is empty!");
            ...
            if (!UIUtil.isApkInstalled(mContext, pkgName, clsName, metroInfo.getItemType())) {
                ...
                if (TextUtils.isEmpty(metroInfo.getApkUrl())) {
                    ...
                    int savedId =dm.getSavedId(mContext);
                    if(savedId == 0){
                    ...
                    Log.i(TAG, "realFileName:" + realFileName);
...
//                                }).show();
                ...
                switch (metroInfo.getItemType()) {
                    case LConstants.METRO_ITEM_APK:
                        ...
                        break;
                    case LConstants.CIBN_ITEM_APK:
                    	Log.d(TAG, "startApk_CIBN");
                    	UIUtil.startApkCibn(mContext, pkgName, clsName, extroInfo, metroInfo.getExtraIntInfo());
                    	TvUtils.setInputToStorage(metroInfo.getPkgName());
                    	break;
    ...
    public void setAddImg() {
        ((LinearLayout) mOriginFrameLayout.findViewById(R.id.app_layout)).setVisibility(View.VISIBLE);
        ((ImageView) mOriginFrameLayout.findViewById(R.id.addIcon)).setVisibility(View.VISIBLE);
        ((ImageView) mOriginFrameLayout.findViewById(R.id.addIcon)).setImageBitmap(BitmapFactory.decodeResource(
                getResources(), R.drawable.add_icon));
        ((ImageView) mOriginFrameLayout.findViewById(R.id.icon)).setVisibility(View.GONE);
        ((TextView) mOriginFrameLayout.findViewById(R.id.title)).setVisibility(View.GONE);
    }
